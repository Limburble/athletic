...
/* ─────────────────────────────────────
   Athletic Sanctuary — shared atoms
   ───────────────────────────────────── */

// Wordmark: Athlet|ic (the italic break)
export const SaMark = ({ size = 20, color = 'var(--sa-ink-1)' }) => (
  <span style={{ display: 'inline-flex', alignItems: 'baseline', lineHeight: 1, color }}>
    <span style={{ fontFamily: 'Newsreader, serif', fontWeight: 300, fontSize: size, letterSpacing: '-0.025em' }}>
      Athlet
    </span>
    <span style={{ fontFamily: 'Newsreader, serif', fontStyle: 'italic', fontWeight: 300, fontSize: size, letterSpacing: '-0.015em' }}>
      ic
    </span>
  </span>
)

// Soft icon set — 1.25px stroke, ample roundness
export const SaIcon = ({ name, size = 20, color = 'currentColor', strokeWidth = 1.25 }) => {
  const paths = {
...
// Mode tag — Definition / Strength / Cool down
export const SaModeTag = ({ mode, style: sx = {} }) => (
...
// Top bar — wordmark + current time + mode indicator
export const SaTopBar = ({ mode = 'def', onMenu, right, pad = 16 }) => {
  const now = new Date()
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `${pad}px 24px` }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
        <SaMark size={20} />
        <span style={{ fontFamily: 'Newsreader, serif', fontStyle: 'italic', fontWeight: 400, fontSize: 13, color: 'var(--sa-ink-3)', letterSpacing: '-0.01em' }}>
          {time}
        </span>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '1px solid var(--sa-rule)', borderRadius: 100,
      }}>
        {items.map(it => {
          const on = tab === it.id
          return (
            <div key={it.id} className="sa-tap" onClick={() => onTab(it.id)} style={{
              ...
              <SaIcon name={it.icon} size={16} color={on ? 'var(--sa-accent)' : 'currentColor'} />
              {on && (
                <span style={{ fontFamily: 'Newsreader, serif', fontStyle: 'italic', fontSize: 14, fontWeight: 400, color: 'var(--sa-ink-1)' }}>
                  {it.label}
                </span>
              )}
            </div>
...
// Soft toggle switch
export const SaSwitch = ({ on, onChange }) => (
  <div className="sa-tap" onClick={() => onChange(!on)} style={{
